...
def help_ant(sheet,num_iter=50000,momentum=0.9,learning_rate=0.79e-3,max_path=None):
    ...
    np.savetxt('deletaC_%s.%s.csv'%(sheet,num_path),np.array([problem.p0,problem.p0+problem.dp]).T,fmt='%d', delimiter=',')
    np.savetxt('deletaNC_%s.%s.csv'%(sheet,num_path),np.array([problem.table.dot(problem.p0),problem.table.dot(problem.p0+problem.dp)]).T,fmt='%d',delimiter=',')
    t3=time.time()
...
def surviving_game(sheet,num_iter=70000,momentum=0.9,learning_rate=0.79e-3,max_path=None,die_rate=0.01):
    '''
    A method to help ants sheduling routes.
    ...
    t2=time.time()
    print 'Do not find Result, cost = %s'%problem.get_cost()
    print 'time elapse: main block: %s, save & load %s.'%(t2-t1,t1-t0)
    problem.fin_problem() #deallocate arrays
...
def test_perturbation(sheet,num_path,num_iter=50000,momentum=0.9,learning_rate=0.79e-3,max_path=None):
    ...
    with open('deletaC_%s.%s.csv'%(sheet,num_path),'rb') as f:
        popt=np.loadtxt(f,dtype='int32',skiprows=0,delimiter=',')[:,1]
...
if __name__=='__main__':
    ...
    #surviving_game(0,max_path=None,learning_rate=0.8e-3,momentum=0.95,die_rate=0.01)

    #run the bad case with 500 nodes.
    #surviving_game(1,max_path=None,learning_rate=0.8e-3,momentum=0.95,die_rate=0.01)

    #run performace test case with 1000 nodes.
    #surviving_game(2,max_path=None,learning_rate=0.8e-3,momentum=0.95,die_rate=0.01)

    #run a perturbation test for case with 1000 nodes.
    #test_perturbation(2,num_path=370,max_path=None,learning_rate=0.8e-3,momentum=0.95)
