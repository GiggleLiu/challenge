...
#@profile
def help_ant(sheet=2,num_iter=50000,momentum=0.9,learning_rate=0.79e-3,max_path=None):
    '''
    ...
    for i in xrange(num_iter):
        ...
        if _fix_path or problem.change_mask.sum()>=max_path:
            _fix_path=True
            allowed_indices=np.where(problem.change_mask)[0]
    ...
    np.savetxt('deletaNC_%s.dat'%(sheet,num_path),np.array([problem.table.dot(problem.p0),problem.table.dot(problem.p0+problem.dp)]).T,fmt='%d')
    t3=time.time()
