...
class BinPath(object):
    ...
    def __init__(self, node_min, node_max, table, p0):
        ...
        self._occ=self.table.dot(self.p0)-self._center
    ...
    def compute_gradient_(self,x,xmask=None,mask_y=True):
        '''Compute the gradient.'''
        if xmask is not None:
            docc=self.table[:,xmask].dot(x)
        else:
            docc=self.table.dot(x)
        occ=self._occ+docc
        if mask_y:
            #mask y version
            occ[abs(occ)<=self._interval]=0
        ...
        grad=self.table.T.dot(occ)#/(self._interval+1))
        return grad
    ...
    def compute_cost(self,x,xmask=None):
        ...
        #return np.sum(overflow[overmask]**2)
...
def get_testcase(which=0):
    if which==1:
        ...
        print table.shape
        #np.savetxt('maxmin1000_int.dat',maxmin,fmt='%d')
        #np.savetxt('p01000_int.dat',p0,fmt='%d')
        #np.savetxt('table1000_int.dat',table,fmt='%d')
...
def method2(sheet=0):
    ...
    for ix in xrange(num_try):
        ...
        #optimize corresponding axis
        print 'x0=%s'%x0[xmask]
        cost0=case.compute_cost(x0[xmask],xmask=xmask)
        opt_cost,opt_x=case.optimize(xmask,x0=x0[xmask])
        x0[xmask]=opt_x
        print xmask
...
def method1(sheet=0):
    ...
    #cope the index with maximum gradient
    for ix in xrange(case.num_path):
        dx=case.compute_gradient(x0,mask_y=True)
        dx[xmask]=0
        xmask.append(np.argmax(abs(dx)))
        xmask=list(set(xmask))
        print xmask
...
def method5(sheet=0):
    ...
    case=get_testcase(sheet)
    #cope the index with maximum gradient
    x0=np.zeros(case.num_path,dtype='float64')
    for ix in xrange(num_try):
        #optimize corresponding axis
        print 'x0=%s'%x0
        for i in xrange(10000):
            cost0=case.compute_cost(x0)
            gradient=case.compute_gradient_(x=x0,mask_y=True)
            pivot=np.argmax(abs(gradient))
            x0[pivot]-=np.sign(gradient[pivot])
            #x0=x0-1e-5*gradient
            opt_cost=case.compute_cost(x0)
            #print cost0,opt_cost
        ...
        if case.check_result(x0):
            print 'Find Result! x=%s, cost=%s, num_change = %s.'%(x0,opt_cost,np.sum(abs(x0)>0))
            break
        else:
            print 'Need more changes, cost=%s, x=%s'%(opt_cost,x0)
        print x0
        pdb.set_trace()
    pdb.set_trace()
