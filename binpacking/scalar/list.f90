...
!
subroutine list_len(list, length)
    type(LINKED_LIST), pointer  :: list
...
! Result:
!
subroutine list_next( elem ,next)
    type(LINKED_LIST), intent(in) pointer :: elem
    type(LINKED_LIST), intent(out), pointer :: next
    next => elem%next
...
!                removed
!
subroutine list_remove( list, elem )
    type(LINKED_LIST), pointer  :: list
    type(LINKED_LIST), pointer  :: elem
...
!     elem       Element in the linked list
!
subroutine list_get_data( elem ,data)
    type(LINKED_LIST),intent(in),pointer :: elem
    type(LIST_DATA),intent(out) :: data
    data = elem%data
end subroutine list_get_data
