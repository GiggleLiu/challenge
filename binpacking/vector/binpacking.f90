module binpacking
    ...
    subroutine dotproduct_redistribute(weight, num_items, size_rem, dim_vector, assign_table)
        implicit none
        integer,intent(in) :: num_items, dim_vector
        real,intent(in) :: weight(num_items,dim_vector)
        real,intent(inout) :: size_rem(:,:)
        integer,intent(out) :: assign_table(num_items)
        real :: av_demand(dim_vector),rem_j(dim_vector)
        integer :: i,j,best_i,best_item_i,num_unset_items,item_table(num_items),item_i,num_bin
        real :: avrem(dim_vector),cur_dot,best_dot  ! Create an array to store remaining space in bins! there can be at most n bins

        !initialize variables
        num_bin=size(size_rem,1)
        num_unset_items=num_items
        item_table=(/(i,i=1,num_items)/)
        av_demand=sum(weight,1)/num_items
        ...
        enddo
        num_bin=j
    end subroutine dotproduct_redistribute
